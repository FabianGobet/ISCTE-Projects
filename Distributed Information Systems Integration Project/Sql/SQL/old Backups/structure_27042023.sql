...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `javaop` AS select `experiencia`.`Id` AS `Id`,`experiencia`.`DataHoraInicio` AS `DataHoraInicio`,`experiencia`.`VariacaoTemperaturaMaxima` AS `VariacaoTemperaturaMaxima`,`experiencia`.`TemperaturaIdeal` AS `TemperaturaIdeal`,`experiencia`.`DataHoraFim` AS `DataHoraFim` from `experiencia` where `experiencia`.`Anulada` = 0 and `experiencia`.`Populada` = 0 and `experiencia`.`DataHoraInicio` <= current_timestamp() and `experiencia`.`DataHoraInicio` >= all (select `tempexp`.`DataHoraInicio` from `experiencia` `tempexp` where `tempexp`.`Anulada` = 0 and `tempexp`.`Populada` = 0 and `tempexp`.`DataHoraInicio` <= current_timestamp()) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
