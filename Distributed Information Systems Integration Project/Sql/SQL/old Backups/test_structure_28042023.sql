...
CREATE TABLE `erroexperiencia` (
  ...
  PRIMARY KEY (`Id`),
  KEY `FK_ERROEXPERIENCIA_EXPERIENCIA` (`IdExperiencia`),
  CONSTRAINT `FK_ERROEXPERIENCIA_EXPERIENCIA` FOREIGN KEY (`IdExperiencia`) REFERENCES `experiencia` (`Id`) ON DELETE CASCADE ON UPDATE CASCADE
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `introduzirAlertaErro` AFTER INSERT ON `erroexperiencia`
 FOR EACH ROW BEGIN
...
/*!50001 CREATE VIEW `javaop` AS SELECT
 1 AS `id_experiencia`,
  1 AS `data_hora_inicio`,
  1 AS `data_hora_fim`,
  1 AS `temperatura_ideal`,
  1 AS `variacao_temperatura`,
  1 AS `fator_alerta_vermelho`,
  1 AS `fator_alerta_laranja`,
  1 AS `fator_alerta_amarelho`,
  1 AS `num_sensores`,
  1 AS `fator_periodicidade`,
  1 AS `tempo_max_periodicidade`,
  1 AS `fator_tamanho_outlier_sample`,
  1 AS `temperatura_maxima`,
  1 AS `temperatura_minima`,
  1 AS `tempo_entre_experiencia`,
  1 AS `segundos_abertura_portas_exterior` */;
SET character_set_client = @saved_cs_client;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `log` (
...
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `parametrosjavaop` AS SELECT
 1 AS `fator_alerta_vermelho`,
  1 AS `fator_alerta_laranja`,
  1 AS `fator_alerta_amarelho`,
  1 AS `num_sensores`,
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `introduzirErroExperiencia`(IN `IdExperiencia` INT, IN `DataHora` DATETIME, IN `Valor` VARCHAR(250))
BEGIN
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `parametrosjavaop` AS select (select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'FATOR_ALERTA_TEMPERATURA' and `pa`.`Identificador` = 1) AS `fator_alerta_vermelho`,(select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'FATOR_ALERTA_TEMPERATURA' and `pa`.`Identificador` = 2) AS `fator_alerta_laranja`,(select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'FATOR_ALERTA_TEMPERATURA' and `pa`.`Identificador` = 3) AS `fator_alerta_amarelho`,(select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'NUM_MAX_SENSORES' and `pa`.`Identificador` = 'NUM_SENSORES') AS `num_sensores`,(select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'PARAMETRO_PERIODICIDADE' and `pa`.`Identificador` = 'FATOR') AS `fator_periodicidade`,(select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'PARAMETRO_PERIODICIDADE' and `pa`.`Identificador` = 'TEMPO') AS `tempo_max_periodicidade`,(select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'TAMANHO_OUTLIER_SAMPLE' and `pa`.`Identificador` = 'FATOR') AS `fator_tamanho_outlier_sample`,(select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'TEMPERATURA' and `pa`.`Identificador` = 'MAXIMA') AS `temperatura_maxima`,(select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'TEMPERATURA' and `pa`.`Identificador` = 'MINIMA') AS `temperatura_minima`,(select `pa`.`Valor` from `micelab`.`parametrosadicionais` `pa` where `pa`.`Agrupador` = 'TEMPO_ENTRE_EXPERIENCIA' and `pa`.`Identificador` = 'TEMPO') AS `tempo_entre_experiencia`,(select `cf`.`SegundosAberturaPortasExterior` from `micelab`.`configuracaolabirinto` `cf` limit 1) AS `segundos_abertura_portas_exterior` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
