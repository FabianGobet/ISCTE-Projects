...
BEGIN
DECLARE mensagemOut VARCHAR(50);

IF(NOT(EXISTS(SELECT * FROM experiencia WHERE experiencia.Id = idExperiencia))) THEN
SET mensagemOut = 'Experiencia não existe';

ELSEIF(EXISTS(SELECT * FROM experiencia WHERE experiencia.Id = idExperiencia AND experiencia.Populada = 0)) THEN
SET mensagemOut = 'Experiencia ainda não populada.';
...
SET mensagemOut = 'Operação concluida com sucesso';

ELSE
SET mensagemOut = 'Frase de confirmação errada';
END IF;
...
/*!50001 VIEW `javaop` AS select `e`.`DataHoraInicio` AS `DataHoraInicio`,`e`.`DataHoraFim` AS `DataHoraFim`,`e`.`LimiteRatosSala` AS `LimiteRatosSala`,`e`.`SegundosSemMovimento` AS `SegundosSemMovimento`,`cfglab`.`SegundosAberturaPortasExterior` AS `SegundosAberturaPortasExterior` from (`micelab`.`configuracaolabirinto` `cfglab` join `micelab`.`experiencia` `e`) where `e`.`DataHoraInicio` >= all (select `micelab`.`experiencia`.`DataHoraInicio` from `micelab`.`experiencia`) */;
