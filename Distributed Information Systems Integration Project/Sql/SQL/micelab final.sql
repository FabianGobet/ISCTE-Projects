...
--
-- Host: 127.0.0.1
-- Generation Time: May 14, 2023 at 08:03 PM
...
SELECT utilizador.Id, utilizador.Tipo INTO idUser,
tempTipo
FROM utilizador
WHERE utilizador.Username = username;
IF ((EXISTS(SELECT * FROM experiencia
WHERE experiencia.Investigador = idUser
AND experiencia.Id = idExp) AND NOT( EXISTS(SELECT * FROM
odorexperiencia oe WHERE oe.IdExperiencia=idExp AND
idOdo=oe.IdOdor))) OR tempTipo='AppAdmin') THEN
...
WHERE experiencia.Investigador = idUser
AND experiencia.Id = idExp) AND NOT( EXISTS(SELECT * FROM
substanciaexperiencia se WHERE se.IdExperiencia=idExp AND
idSub=se.IdSubstancia))) OR tempTipo='AppAdmin') THEN

INSERT INTO substanciaexperiencia (IdSubstancia,
IdExperiencia, NumeroRatos)
...
FROM utilizador u where u.Username=(SELECT SUBSTRING_INDEX(USER(),'@',1))$$
...
UPDATE `experiencia` SET `Anulada` = '1' WHERE
`experiencia`.`Id` = idExperiencia;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteOdoresExperiencia` (IN `idExp` INT)   BEGIN
DECLARE username VARCHAR(30);
DECLARE idUser INT;
DECLARE tempTipo VARCHAR(100);
...
SELECT utilizador.Id, utilizador.Tipo INTO idUser, tempTipo
FROM utilizador
WHERE utilizador.Username = username;

IF (EXISTS(SELECT * FROM experiencia e WHERE e.Id=idExp
AND e.Investigador=idUser) OR tempTipo='AppAdmin') THEN
DELETE FROM odorexperiencia WHERE
odorexperiencia.IdExperiencia = idExp;

END IF;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteSubstanciaExperiencia` (IN `idExp` INT)   BEGIN
...
IF ((EXISTS(SELECT * FROM experiencia e WHERE e.Id=idExp
AND e.Investigador=idUser)) OR tempTipo='AppAdmin') THEN
DELETE FROM substanciaexperiencia WHERE
substanciaexperiencia.IdExperiencia = idExp;

END IF;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteUtilizador` (IN `username` VARCHAR(150))   BEGIN

DELETE FROM utilizador WHERE utilizador.username = username;

SET @query1 = CONCAT('DROP USER "',Username,'"@"%"');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `experienciaPopulada` (IN `idExperiencia` INT)   UPDATE experiencia SET experiencia.Populada=1 WHERE
experiencia.Id=idExperiencia$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `finalizarExperiencia` (IN `IdExperiencia` INT(11))   BEGIN
 update experiencia set DataHoraFim =
current_timestamp where Id = IdExperiencia and
DataHoraFim is null;
END$$
...
INSERT INTO `experiencia` (`Id`, `Descricao`,
`Investigador`, `DataHoraInicio`, `NumeroRatos`,
`LimiteRatosSala`, `SegundosSemMovimento`,
`VariacaoTemperaturaMaxima`, `TemperaturaIdeal`,
`DataHoraFim`, `Populada`, `Anulada`) VALUES (NULL,
Descricao, idUtilizador, nvl(DataHoraInicio,
CURRENT_TIMESTAMP), NumeroRatos, LimiteRatosSala,
SegundosSemMovimento, VariacaoTemperaturaMaxima,
TemperaturaIdeal, NULL, '0', '0');
SELECT last_insert_id() FROM experiencia;
END$$
...
IF(podeInserirAlerta(IdTipoAlerta, IdExperiencia)) THEN
...
SELECT COUNT(*) INTO corredor
 FROM corredor
 WHERE corredor.SalaEntrada = salaDeEntrada AND
corredor.SalaSaida = salaDeSaida ;
IF(corredor > 0) THEN
 INSERT INTO medicaopassagem (idExperiencia, DataHora,
SalaSaida, SalaEntrada)
 VALUES (iDExp, horaPassagem, salaDeSaida,
salaDeEntrada);
ELSE
SET valor = CONCAT('Corredor nao existe na experiencia:
', idExp ,'; entrada: ', salaDeEntrada, '; saida:
',salaDeSaida);
CALL introduzirErroExperiencia(idExp,horaPassagem,valor);
END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `mudarExperiencia` (IN `idExperiencia` INT, IN `Descricao` TEXT, IN `DataHoraInicio` INT, IN `NumeroRatos` INT, IN `LimiteRatosSala` INT, IN `SegundosSemMovimento` INT, IN `VariacaoTemperaturaMaxima` DECIMAL(4,2), IN `TemperaturaIdeal` DECIMAL(4,2))   BEGIN
IF !(CURRENT_TIMESTAMP>= (SELECT e.DataHoraInicio FROM
experiencia e WHERE e.Id=idExperiencia)) THEN
IF(!(DataHoraInicio IS NULL) AND DataHoraInicio >=
CURRENT_TIMESTAMP) THEN
UPDATE experiencia e SET e.DataHoraInicio =
DataHoraInicio WHERE e.Id = idExperiencia;
END IF;
IF(!(NumeroRatos IS NULL)) THEN
UPDATE experiencia e SET e.NumeroRatos =
NumeroRatos WHERE e.Id = idExperiencia;
UPDATE medicaosala ms SET
ms.NumeroRatos=NumeroRatos WHERE
ms.IdExperiencia=idExperiencia;
END IF;
IF(!(Descricao IS NULL OR Descricao='')) THEN
UPDATE experiencia e SET e.Descricao=Descricao
WHERE e.Id=idExperiencia;
END IF;
IF(!(LimiteRatosSala IS NULL OR LimiteRatosSala=0))
THEN
UPDATE experiencia e SET
e.LimiteRatosSala=LimiteRatosSala WHERE
e.Id=idExperiencia;
END IF;
IF(!(SegundosSemMovimento IS NULL OR
SegundosSemMovimento=0)) THEN
UPDATE experiencia e SET
e.SegundosSemMovimento=SegundosSemMovimento WHERE
e.Id=idExperiencia;
END IF;
IF(!(VariacaoTemperaturaMaxima IS NULL)) THEN
UPDATE experiencia e SET
e.VariacaoTemperaturaMaxima=VariacaoTemperaturaMaxima
WHERE e.Id=idExperiencia;
END IF;
IF(!(TemperaturaIdeal IS NULL)) THEN
UPDATE experiencia e SET
e.TemperaturaIdeal=TemperaturaIdeal WHERE
e.Id=idExperiencia;
END IF;
END IF;
END$$
...
IF(NOT(EXISTS(SELECT * FROM experiencia WHERE
experiencia.Id = idExperiencia))) THEN
SET resultado = 'Experiencia não existe';
ELSEIF(EXISTS(SELECT * FROM experiencia WHERE
experiencia.Id = idExperiencia AND experiencia.Populada =
0 AND experiencia.DataHoraFim IS NULL)) THEN
SET resultado = 'Experiencia ainda não
populada/terminada.';
ELSEIF(confirmacao = 'Apagar e repor os dados') THEN
UPDATE experiencia SET experiencia.Populada=0 WHERE
experiencia.Id=idExperiencia;
DELETE FROM alerta WHERE alerta.IdExperiencia =
idExperiencia;
DELETE FROM medicaopassagem WHERE
medicaopassagem.IdExperiencia = idExperiencia;
DELETE FROM medicaotemperatura WHERE
medicaotemperatura.IdExperiencia = idExperiencia;
DELETE FROM medicaosala WHERE medicaosala.IdExperiencia =
idExperiencia;
INSERT INTO medicaosala (IdExperiencia,Sala,NumeroRatos)
(SELECT experiencia.Id,1,experiencia.NumeroRatos FROM
experiencia WHERE experiencia.Id=idExperiencia);
...
CREATE DEFINER=`root`@`localhost` FUNCTION `podeInserirAlerta` (`IdTipoAlerta` INT(11), `idExp` INT) RETURNS TINYINT(1) UNSIGNED ZEROFILL  BEGIN 

RETURN ((SELECT COUNT(*)
FROM ALERTA
WHERE idExp = IdExperiencia AND IdTipoAlerta = IdTipoAlerta AND TIMESTAMPDIFF(SECOND, DataHora, current_timestamp) <= (SELECT CAST(VALOR AS INT)
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `validarDadosExperienciaUpdate` BEFORE UPDATE ON `experiencia` FOR EACH ROW BEGIN
...
,`fator_alerta_amarelo` varchar(30)
