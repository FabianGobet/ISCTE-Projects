...
void init_ipc() {
    ...
    sem_id = msgget(IPC_KEY,1, IPC_CREAT | 0600 | IPC_EXCL);
    exit_on_error(sem_id, "S1) Semáforo com a Key definida não pode ser iniciado com o valor 1");
    // O semáforo em questão será usado com o padrão “Mutex”, pelo que será iniciado com o valor 1;
...
void init_database() {
    ...
    // • Lê o ficheiro FILE_ENFERMEIROS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->enfermeiros e db->num_enfermeiros. Se não o conseguir, dá erro e termina com exit status 1;
     int size_fich_enf = read_binary(FILE_CIDADAOS,db.enfermeiros, MAX_ENFERMEIROS*sizeof(Enfermeiro));
...
void trata_mensagem_cidadao() {
    ...
    // } else if (...) {
      ...
      else if(mensagem.pedido=CANCELAMENTO){
...
void processa_pedido() {
    ...
    for(int i=0; i<db.num_cidadaos; i++){
      if(mensagem.dados.num_utente==db.cidadaos[i].num_utente && (strcmp(mensagem.dados.nome,db.cidadaos[i].nome))==0){
        encontrou_cid=i;
        resposta.dados.cidadao=db.cidadao[i];
        if(db.cidadao[i].estado_vacinacao==2)
          resposta.dados.status=VACINADO;
        if((db.cidadao[i].PID_cidadao>0){
          resposta.dados.status=EMCURSO;
          break;
        }
        else{
          db.cidadao[i].PID_cidadao=mensagem.dados.PID_cidadao;
          break;
        }
      }
    }
    if(encontrou_cid==-1){
      ...
      erro("S5.1) Cidadão %d, %s  não foi encontrado na BD Cidadãos", mensage.dados.num_utente, mensagem.dados.nome);
    }
    else
      sucesso("S5.1) Cidadão %d, %s encontrado, estado_vacinacao=%d, status=%d", bd.cidadaos[encontrou_cid].num_utente, bd.cidadaos[encontrou_cid].nome, bd.cidadaos[encontrou_cid].estado_vacinacao, resposta.dados.status);
    ...
    int encontrou_enf=-1;
    if(resposta.dados.status!=DESCONHECIDO && resposta.dados.status!=VACINADO && resposta.dados.status!=EMCURSO && encontrou_cid!=-1){
      for(int i=0; i<num_enfermeiros; i++){
        ...
        if((strcmp(db.enfermeiros[i].CS_enfermeiro,cid_local))==0){
          ...
          if(db.enfermeiros[i].disponibilidade==0){
            ...
            sucesso("S5.2) Enfermeiro do CS %s encontrado, disponibilidade=%d, status=%d", db.cidadaos[encontrou_cid].localidade, db.enfermeiros[i].disponibilidade, resposta.dados.status);
            break;
          }
          else{
            reserva_vaga(encontrou_cid,encontrou_enf);
            if(vaga_ativa==-1){
              resposta.dados.status=AGUARDAR;
              ...
              sucesso("S5.3) Foi reservada a vaga %d para vacinação, status=%d", vag, resposta.dados.status);
              break;
            }
          }
        }
...
void vacina() {
    ...
    if(pid_filho==0)
      ...
      bd.vagas[vaga_ativa].PID_filho=pid_filho;
    debug(">");
}
    ...
    for(int i=0; i<MAX_VAGAS; i++)
      if(db.vagas[i].index_cidadao!=-1)
        if(db.cidadaos[db.vagas[i].index_cidadao].num_utente==mensagem.dados.num_utente && (strcmp(mensagem.dados.nome,db.cidadaos[db.vagas[i].index_cidadao].nome))==0){
          ...
          kill(SIGTERM, db.vagas[i].PID_filho);
...
void termina_servidor(int sinal) {
    ...
    save_binary(FILE_ENFERMEIROS,db.cidadaos,db.num_cidadaos*sizeof(Cidadao));
