...
void processa_pedido() {
    ...
    int encontrou_enf=-1;
    // • So vale a pena procurar enfermeiro caso tenhamos encontrado cidadao
    if(encontrou_cid!=-1 && resposta.dados.status!=NAOHAENFERMEIRO){ // so vai procurar enfermeiros caso isto se verifique
      if(resposta.dados.status!=DESCONHECIDO && resposta.dados.status!=VACINADO && resposta.dados.status!=EMCURSO){
        for(int i=0; i<db->num_enfermeiros; i++){
          char cid_local[102]="CS"; // cria uma string com "CS" no inicio para depois fazer comparaçao com localidade do enfermeiro
...
void termina_servidor(int sinal) {
   ...
   if(semctl(sem_id,0,GETVAL)>0)
    ...
    for(int i=0; i<MAX_VAGAS; i++)
      if(db->vagas[i].index_cidadao!=-1){
        db->cidadaos[db->vagas[i].index_cidadao].PID_cidadao=-1;
        db->enfermeiros[db->vagas[i].index_enfermeiro].disponibilidade=1;
        kill(db->vagas[i].PID_filho,SIGTERM); 
      }
