 ...
 **   • A descrição do módulo está feita através de comentários no decorrer do código
 **   • É feito o uso de semáforos sempre que é feito um acesso para leitura ou escritura na BD. Para tal fiz um diagrama em arvore com as possiveis ramificações (precursos) que o código pode tomar.
 **     Em todas as ramificações o numero de sem_mutex_up() é igual ao numero de sem_mutex_down() à exceção do caso em que o servidor é terminado, onde haverá um sem_mutex_up() a mais
 **     Para o efeito, estarei disposto a fornecer um desenho com o diagrama e a lógica usados para implementar o uso dos semáforos, caso necessário
 **
 ******************************************************************************/
#include "common.h"
#include "utils.h"
...
void init_ipc() {
    ...
    // Todos estes elementos têm em comum serem criados com a KEY IPC_KEY definida em common.h (alterar esta KEY para ter o valor do nº do aluno, como indicado nas aulas), e com permissões 0600. Se qualquer um destes elementos IPC já existia anteriormente, dá erro e termina com exit status 1. Esta função, em caso de sucesso, preenche as variáveis globais respetivas msg_id, sem_id, e shm_id;
    // • uma fila de mensagens IPC;
    msg_id = msgget(IPC_KEY, IPC_CREAT | 0600 | IPC_EXCL);
    exit_on_error(msg_id, "S1) Fila de Mensagens com a Key definida já existe ou não pode ser criada");
    
    // • um array de semáforos IPC de dimensão 1;
    sem_id = semget(IPC_KEY,1,IPC_CREAT | 0600 | IPC_EXCL);
    exit_on_error(sem_id, "S1) Semáforo com a Key definida já existe ou não pode ser criada");
    
    // O semáforo em questão será usado com o padrão “Mutex”, pelo que será iniciado com o valor 1;
    int stat_sem = semctl(sem_id, 0, SETVAL, 1); 
    exit_on_error(stat_sem, "S1) Semáforo com a Key definida não pode ser iniciado com o valor 1");
    

    // • uma memória partilhada IPC de dimensão suficiente para conter um elemento Database.
    shm_id = shmget(IPC_KEY,sizeof(Database), IPC_CREAT | 0600 | IPC_EXCL);
...
void init_database() {
    ...
    // • Lê o ficheiro FILE_CIDADAOS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->cidadaos e db->num_cidadaos. Se não o conseguir, dá erro e termina com exit status 1;
    // • O numero maximo que o ficheiro poderá ter será tanto quanto o numero maximo de cidadaos(MAX_CIDADAOS) possiveis na db dos cidadoes
    int size_fich_cid = read_binary(FILE_CIDADAOS,db->cidadaos,MAX_CIDADAOS*sizeof(Cidadao));
    db->num_cidadaos = size_fich_cid / sizeof(Cidadao); 
    
    // • Lê o ficheiro FILE_ENFERMEIROS e armazena o seu conteúdo na base de dados usando a função read_binary(), assim preenchendo os campos db->enfermeiros e db->num_enfermeiros. Se não o conseguir, dá erro e termina com exit status 1;
    int size_fich_enf = read_binary(FILE_ENFERMEIROS,db->enfermeiros,MAX_ENFERMEIROS*sizeof(Enfermeiro));
    db->num_enfermeiros = size_fich_enf / sizeof(Enfermeiro);
...
void trata_mensagem_cidadao() {
    ...
    // S4) O comportamento do processo Servidor agora irá depender da variável global mensagem enviada pelo processo Cidadão no campo pedido
      ...
      if(mensagem.dados.pedido==PEDIDO){
        ...
        cancela_pedido();
        // • Neste passo é necessário fazer um reset ao status da resposta senão pode ocorrer um problema caso venha o memso cidaão que cancelou logo de seguida.
...
void processa_pedido() {
    ...
    // • Procura o cidadao na BD de Cidadaos, e caso encontre guarda o seu indice na variavel encontrou_cid
    // • Se a localidade do cidadão for "LocalidadeSemEnfermeiro", então define logo status da resposta
    // • Apos encontrar o cidadão e testar faz os testes, atribui os respetivos valores e sai logo do ciclo for.
    int encontrou_cid=-1;
    for(int i=0; i<db->num_cidadaos; i++){
      if(mensagem.dados.num_utente==db->cidadaos[i].num_utente && (strcmp(mensagem.dados.nome,db->cidadaos[i].nome))==0){
        encontrou_cid=i;
        debug("%d", db->cidadaos[i].PID_cidadao);
        ...
        else if(strcmp(db->cidadaos[i].localidade,"LocalidadeSemEnfermeiro")==0){
    ...
    // • Se a variavel local encontrou_cid continuar a -1 é porque nunca encontrou um cidadao, e entao atribui o status desconhecido à resposta.
    if(encontrou_cid==-1){
      ...
      sucesso("S5.1) Cidadão %d, %s encontrado, estado_vacinacao=%d, status=%d", db->cidadaos[encontrou_cid].num_utente, db->cidadaos[encontrou_cid].nome, db->cidadaos[encontrou_cid].estado_vacinacao, resposta.dados.status);
    ...
    // • Similarmente ao cidadão, usamos uma variavel encontrou_enf para guardar o indice do enfermeiro, caso o encontremos
    int encontrou_enf=-1;
    // • So vale a pena procurar enfermeiro caso tenhamos encontrado cidadao, e o status nao seja nenhum dos descritos abaixo
    if(encontrou_cid!=-1 && resposta.dados.status!=DESCONHECIDO && resposta.dados.status!=VACINADO && resposta.dados.status!=EMCURSO && resposta.dados.status!=NAOHAENFERMEIRO){
      // • Como a localidade dos enfermeiros tem "CS" como prefixo, é preciso criar uma variavel local para fazer a comparação com a localidade do cidadao encontrado, adicionando o prefixo "CS" à localidade do mesmo
      char cid_local[102]="CS"; 
      strcat(cid_local,db->cidadaos[encontrou_cid].localidade);
      for(int i=0; i<db->num_enfermeiros; i++){
        if((strcmp(db->enfermeiros[i].CS_enfermeiro,cid_local))==0){
          encontrou_enf=i;
          if(db->enfermeiros[i].disponibilidade==0){
            resposta.dados.status=AGUARDAR;
            sucesso("S5.2) Enfermeiro do CS %s encontrado, disponibilidade=%d, status=%d", db->cidadaos[encontrou_cid].localidade, db->enfermeiros[encontrou_enf].disponibilidade, resposta.dados.status);
            // • Caso o enfermeiro esteja ocupado é preciso repor o PID_cidadao na BD do cidadao encontrado, caso contrario no proximo pedido ele assume o status EMCURSO de forma errada.
            db->cidadaos[encontrou_cid].PID_cidadao=-1;
            break;
          }
          else{
            if(reserva_vaga(encontrou_cid,encontrou_enf)==-1){
              resposta.dados.status=AGUARDAR;
              // • Caso não haja vaga é preciso repor o PID_cidadao na BD do cidadao encontrado, caso contrario no proximo pedido ele assume o status EMCURSO de forma errada.
              db->cidadaos[encontrou_cid].PID_cidadao=-1;
              sucesso("S5.2) Enfermeiro do CS %s encontrado, disponibilidade=%d, status=%d", db->cidadaos[encontrou_cid].localidade, db->enfermeiros[encontrou_enf].disponibilidade, resposta.dados.status);
      ...
      // • Se no final do ciclo for não foi encontrado nenhum enfermeiro para a localidade do nosso cidaão, entao a resposta assume naturalmente o status NAOHAENFERMEIRO
      resposta.dados.status=NAOHAENFERMEIRO;
      erro("S5.2) Enfermeiro do CS %s não foi encontrado na BD Cidadãos", db->cidadaos[encontrou_cid].localidade);   
    }
    ...
    if(resposta.dados.status==OK){
      ...
      // • É necessario fazer um "reset" ao status da resposta para que o proximo cidadao que venha e exista na BD nao tenha um status que possa inibir o codigo prévio de fazer uma procura pelo enfermeiro da respetiva localidade
      resposta.dados.status=0;
      sem_mutex_up;
    }
    debug(">");
...
void servidor_dedicado() {
  ...
  // S7.2) Envia a resposta para o Cidadao, chamando a função envia_resposta_cidadao(). Implemente também esta função, que envia a mensagem resposta para o cidadao, contendo os dados do Cidadao preenchidos em S5.1 e o campo status = OK;
    ...
    // • Para facilitar o código que segue, são criadas duas variaveis respeitantes ao index do enfermeiro e ao index do cidadão
...
int reserva_vaga(int index_cidadao, int index_enfermeiro) {
    ...
    // • Inicia com o valor de vaga_ativa a -1
    vaga_ativa = -1;
...
void liberta_vaga(int index_vaga) {
    debug("<");
    // • Com a libertação da vaga o enfermeiro passa a disponivel e o campo PID_cidadao do cidadao na BD passa a -1 para permitir uma proxima asserção correta da acção a tomar
    db->enfermeiros[db->vagas[index_vaga].index_enfermeiro].disponibilidade=1;
...
void cancela_pedido() {
    ...
    for(int i=0; i<MAX_VAGAS; i++){
      if(db->vagas[i].index_cidadao!=-1){
        if(db->cidadaos[db->vagas[i].index_cidadao].num_utente==mensagem.dados.num_utente && (strcmp(mensagem.dados.nome,db->cidadaos[db->vagas[i].index_cidadao].nome))==0){
          ...
          sucesso("S10.2) Enviado sinal SIGTERM ao Servidor Dedicado com PID=%d", db->vagas[i].PID_filho);
          return;
        }
      }
    }
    // • Mensagem de erro caso nao seja encontrado a respetiva sessão nas vagas da BD
...
void termina_servidor(int sinal) {
   ...
   if(semctl(sem_id,0,GETVAL)>0)
    ...
    for(int i=0; i<MAX_VAGAS; i++)
      if(db->vagas[i].index_cidadao!=-1){
       // • Quando encontrar um sessão ativa manda o sinal SIGTERM ao PID_filho,e este ira libertar a vaga, que por sua vez irá repor a disponibilidade do enfermeiro e o PID_cidadao na BD
       kill(db->vagas[i].PID_filho,SIGTERM); 
      }
...
void termina_servidor_dedicado(int sinal) {
   ...
   // • EXTRA: existe uma probabilidade infima de acontecer um singterm depois do tempo de consulta. É improvavel, mas caso aconteça esta condição permite guardar os dados relativos ao enfermeiro e ao cidadao sem problemas.
  ...
  // • EXTRA: No caso de a vacinação já ter sido terminada, mostra esta mensagem de erro
  erro("S12.3) Vacinação já está em curso. Não é possivel interromper."); //extra: no caso de a vacina ja ter sido terminada
}
